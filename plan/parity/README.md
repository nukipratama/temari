...
| PS11 | Settings | in-review | [#690](https://github.com/nukipratama/temari/pull/690) | 3 | [PS11-settings](slices/PS11-settings.md); both `PP1`-deferred reflows resolved — **#10's premise was false** (`SettingsScreen.tsx:473-490` draws two inputs in that grid, not the three `PP1` recorded, so the wide step was portable verbatim with no data change), #11 landed with the `AccountActions` restructure into the prototype's button pair; training preferences becomes the prototype's always-open card on the three controls Onboarding already shares with it (`TrainingPreferencesDisclosure` → `TrainingPreferencesCard`) and moves above the zones disclosure; the zones editor drops ten bound inputs to five, since `UpdateHrZonesRequest` already forces `hi[i] == lo[i+1]` (client-only, request class byte-unchanged); found `epic/mobile-ux-port` **already red at 7a8d54c9** on an orphaned `foreground/0.06` grounds registration, fixed in its own commit; coverage 97.37% → **97.52%**, up on all four axes; no browser pass run |
