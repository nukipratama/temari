...
| PP4 | Demo seed completeness | todo | — | main | after every screen slice. **Known gap for P30, found by `PS9`**: `demo:seed` grants 21 unlocks but `UnlockGrantedNotification` is `ShouldQueue`, so with no worker running the demo inbox holds only 2 rows and P12's unlock surface is invisible. |
