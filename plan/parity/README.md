...
| PS7 | History | in-review | [#693](https://github.com/nukipratama/temari/pull/693) | 2 | [PS7-history](slices/PS7-history.md); P3's load-older is a real `?weeks=` page (`FeedQuery::weekWindow`, week-grained), replacing the client reveal and the `MAX_RUNS=365` cap; the calendar is rebuilt onto the prototype's per-week box grid with a tap-to-expand recap disclosure, and **P12's kartu badge lands inside that disclosure** (week's rarest card), never on a day cell; one shared `HistoryHeader` for both `?view=` halves, which had drifted apart; week rows total the whole Mon-Sun week while the month meta stays month-scoped (the prototype's own meta sums its rows and so counts July days in August) — recorded, not followed; one plan error found — **`EmptyRunsState` is not on History** (`Home.tsx` is its only consumer), so the brief's second `FaceIcon` placement is `EmptyPanel` at 48; `reference.md` §9 verified accurate throughout; coverage flat 97.41% with branches 91.21 → 91.31 |
