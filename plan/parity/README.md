...
| PS5 | Race | in-review | [#694](https://github.com/nukipratama/temari/pull/694) | 1 | [PS5-race](slices/PS5-race.md); the prototype's section list, with the schedule/race tabs moved **below** the intro where `RaceGoalScreen` puts them; `PlanRaceTabs` onto the prototype's segmented track (`HistoryNav`'s shape, from the same prototype element), which orphaned **`SectionTabs`** — deleted, as `PP1` predicted; three prototype-named components out of the page body (`RaceCard`, `ProjectionBlock`, `RaceGoalForm`); P26 verified rather than redone, and P3 was already satisfied — the mockup's three dead controls were live before this slice; the `AiReplanPill` cooldown deliberately not ported (no rate limit exists on `POST /race`; replan lives on Plan behind its own); one plan error found — **`cut-list.md` §1's Race row is wrong about where the cut chart went**, `PS6` built its own so `race/CtlTrendChart.tsx` is now orphaned, not "surviving on Trends"; `reference.md` §7 verified accurate throughout; coverage 97.55% → **97.63%**, up on three of four axes |
