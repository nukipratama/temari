...
| PS5 | Race | todo | — | wt | |
| PS6 | Trends | in-review | [#692](https://github.com/nukipratama/temari/pull/692) | 1 | [PS6-trends](slices/PS6-trends.md); the prototype's four blocks, P3's tabs now really re-window narration + chart + tiles + chips, P14/P15's wrapping chip row; **`PP3` left a fifth block** — `StreakBadge` — which P25 does not allow and P27 keeps only as a chip, so it is deleted and folded into `FitnessPanel`; `FitnessTrend`→`FitnessPanel` and `NarrationHeadline`→`NarrationCard` on the prototype's vocabulary; one backend change, `firstEarnedDatesForUser`→`firstEarnedBadgesForUser` carrying rarity so the chip medals tint from real data; one plan finding — `reference.md` §8's absent-list misses `StreakBadge` entirely; coverage 97.41% → **97.55%** |
| PS7 | History | todo | — | wt | |
