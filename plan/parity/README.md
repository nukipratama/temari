...
| P5 | **Responsive: adopt the prototype's own model.** ~~The prototype has zero responsive classes.~~ **Corrected 2026-08-31, see §6** — it has a real responsive layer built on **container queries** (`@min-[900px]:`, 93 occurrences across all 11 screens), which a `sm:`/`md:`/`lg:` grep does not find. `PhoneFrame.tsx:47` sets `[container-type:inline-size]`, so it fires at the desktop and wide viewports but not tablet. **One breakpoint, at 900px.** Below it: the single mobile column as drawn. Above it: a centred **760px** column (**520px** Onboarding, **440px** Login's auth card), reduced top padding (`pt-16` → `pt-6`), and the four wide-only layouts the prototype actually draws — Login's feature list stacked → `grid-cols-3` and its CTA row → `inline-grid grid-cols-2`, Settings' `grid-cols-2` → `grid-cols-4`, and Profile's right-aligned block revealed by `hidden @min-[900px]:block`. Collapse only desktop layout the prototype does *not* specify. |
...
|---|---|---|---|---|---|
| PP0 | Full-scroll reference capture | in-review | [#PR](https://github.com/nukipratama/temari/pull/PR) | main | [reference.md](reference.md) |
