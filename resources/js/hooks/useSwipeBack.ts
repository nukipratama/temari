    ...
    useEffect(() => {
        ...
        const onTouchMove = (event: TouchEvent) => {
            ...
            if (page && !reducedMotion) {
                ...
                page.style.opacity = String(
                    Math.max(0.4, 1 - dx / window.innerWidth),
                );
            }
        };
        ...
        const onTouchEnd = (event: TouchEvent) => {
            ...
            const commit =
                locked &&
                (dx > window.innerWidth * COMMIT_RATIO ||
                    dx / elapsed > COMMIT_VELOCITY);
            ...
            if (commit) {
                ...
                release(
                    ...
                    reducedMotion ? '' : `translateX(${window.innerWidth}px)`,
                );
                window.history.back();
            } else if (locked) {
