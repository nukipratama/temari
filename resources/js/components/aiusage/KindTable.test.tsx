...
describe('KindTable', () => {
    it('renders one row per kind under its own heading', () => {
        render(<KindTable rows={[row(), unmeasured]} grandTotal={880} currency="USD" />);
    ...
    it('renders the agent line from whichever of the three measures arrived', () => {
        render(<KindTable rows={[row({ cached_pct: null, reasoning_pct: null })]} grandTotal={880} currency="USD" />);

        expect(screen.getByText('3.5 langkah')).toBeInTheDocument();
    });

    it('treats a measured zero as measured, not as missing', () => {
        render(<KindTable rows={[row({ avg_steps: 0, cached_pct: 0, reasoning_pct: 0 })]} grandTotal={880} currency="USD" />);

        expect(screen.getByText('0.0 langkah · 0% cache · 0% reasoning')).toBeInTheDocument();
    });

    it('shows latency as avg/max in seconds', () => {
        render(<KindTable rows={[row({ avg_latency_ms: 1400, max_latency_ms: 5200 })]} grandTotal={880} currency="USD" />);

        expect(screen.getByText('1 / 5 dtk')).toBeInTheDocument();
    });

    it('falls back to the average when no max latency was recorded', () => {
        render(<KindTable rows={[row({ avg_latency_ms: 3000, max_latency_ms: null })]} grandTotal={880} currency="USD" />);

        expect(screen.getByText('3 / 3 dtk')).toBeInTheDocument();
    });

    it('shows an em dash in the latency cell when latency was never measured', () => {
        render(<KindTable rows={[row({ avg_latency_ms: null, max_latency_ms: null })]} grandTotal={880} currency="USD" />);

        const latencyCell = screen.getByText('run-insight').closest('tr')?.querySelector('td:nth-child(7)');
    ...
    it('shows an em dash in the truncated cell when nothing was cut off', () => {
        ...
        const truncatedCell = screen.getByText('run-insight').closest('tr')?.querySelector('td:nth-child(8)');
        expect(truncatedCell?.textContent).toBe('—');
    });

    it('reports the truncated count with its rate once any call was cut off', () => {
        render(<KindTable rows={[row({ calls: 4, truncated_calls: 1 })]} grandTotal={880} currency="USD" />);
    ...
    it('avoids dividing by a zero grand total or a zero call count', () => {
        render(<KindTable rows={[row({ calls: 0, truncated_calls: 0 })]} grandTotal={0} currency="USD" />);
