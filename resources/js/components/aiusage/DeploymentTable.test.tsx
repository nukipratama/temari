...
describe('DeploymentTable', () => {
    ...
    it('shows the per-deployment input/output rate as a paired cell', () => {
        render(<DeploymentTable rows={[row({ deployment: 'nuki-5.2', inputPer1m: 1.75, outputPer1m: 14 })]} currency="USD" />);
    ...
    it('shows an em dash for a deployment with no configured rate', () => {
        render(<DeploymentTable rows={[row({ deployment: 'mystery-deploy', inputPer1m: null, outputPer1m: null })]} currency="USD" />);
    ...
    it('shows an em dash when only one side of the rate pair is configured', () => {
        render(<DeploymentTable rows={[row({ deployment: 'half-priced', inputPer1m: 1.75, outputPer1m: null })]} currency="USD" />);

        const rateCell = screen.getByText('half-priced').closest('tr')?.querySelector('td:nth-child(2)');
        expect(rateCell?.textContent).toBe('—');
    });
