...
import type { ActivityDetail, AnalysisPayload, BriefingResult } from '@/types/inertia';
...
function briefingWith(content: string, extra: Partial<AnalysisPayload> = {}): BriefingResult {
    return {
        vibeState: 'pumped',
        vibeLabel: 'Membara',
        vibeEmoji: '💥',
...
describe('KataTemariCard', () => {
    it('renders the section label and a title-only voice', () => {
        render(<KataTemariCard briefing={briefingWith('“Lari santai aja hari ini.”')} pose="observational" lastRun={null} />);
    ...
    it('splits the merged voice into a session title and Temari’s reasoning', () => {
        ...
        expect(screen.getByText(/Dua sesi terakhirmu easy semua/)).toBeInTheDocument();
    });

    it('renders a weather chip from the last run', () => {
        render(<KataTemariCard briefing={briefingWith('Tempo ringan.')} pose="observational" lastRun={runWithWeather} />);
        expect(screen.getByText('28°C · 70%')).toBeInTheDocument();
    });

    it('emits no chip when there is no last run', () => {
        render(<KataTemariCard briefing={briefingWith('Tempo ringan.')} pose="observational" lastRun={null} />);
    ...
    it('disables "Saran lain" and shows a countdown while on cooldown', () => {
        render(
            <KataTemariCard
                briefing={briefingWith('Tempo ringan.', { retry_after_seconds: 900 })}
                pose="observational"
                lastRun={null}
            />,
        );
    ...
    it('shows a long body in full, with nothing to expand', () => {
        ...
        render(<KataTemariCard briefing={briefingWith(`Tempo ringan hari ini.\n\n${longBody}`)} pose="observational" lastRun={null} />);
