...
describe('StravaAction', () => {
    it('renders its child while Strava is enabled', () => {
        ...
        render(<StravaAction><button type="button">Sync sekarang</button></StravaAction>);
        expect(screen.getByRole('button', { name: 'Sync sekarang' })).toBeInTheDocument();
    });
    ...
    it('hides its child entirely while paused, rather than disabling it', () => {
        ...
        const { container } = render(<StravaAction><button type="button">Sync sekarang</button></StravaAction>);
