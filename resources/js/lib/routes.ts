 ...
 */
export function analysisTriggerUrl(analysis: Pick<AnalysisPayload, 'type' | 'subject_id' | 'discriminator'>): string {
    const base = `/api/analyses/${analysis.type}/${analysis.subject_id}/trigger`;

    return analysis.discriminator ? `${base}?discriminator=${encodeURIComponent(analysis.discriminator)}` : base;
}
