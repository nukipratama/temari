...
const pointToPointPolyline = polylineCodec.encode([
    [0, 0],
    [0.01, 0.01],
]);
...
describe('drawShareCard', () => {
    ...
    it.each(layouts)(
        'paints the %s layout at the fixed story resolution',
        async (layout) => {
            const ctx = makeCtx();
            const canvas = {
                width: 0,
...
describe('drawShareCard — edge / branch cases', () => {
    ...
    it('thins the route stroke for a longer distanceKm and keeps it fixed without one', async () => {
        ...
        expect(Math.max(...longRun.widths)).toBeLessThan(
            Math.max(...shortRun.widths),
        );
    });
    ...
    it('puts elevation in the stat grid and location + date in the bottom context strip', async () => {
        ...
        expect(ctx.fillText).toHaveBeenCalledWith(
            expect.stringContaining('30 Mei 2026'),
            expect.any(Number),
            expect.any(Number),
        );
        expect(ctx.fillText).not.toHaveBeenCalledWith(
