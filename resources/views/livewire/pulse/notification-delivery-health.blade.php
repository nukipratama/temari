<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    ...
    <x-pulse::scroll :expand="$expand" wire:poll.30s="">
        ...
        @if ($channels !== [])
            <div class="mb-4">
                <div class="text-label-micro text-text-3 mb-1">Per channel</div>
                <div class="space-y-1">
                    @foreach ($channels as $channel)
                        <div class="flex items-center justify-between rounded-sm bg-muted px-2 py-1 text-xs">
                            <span class="truncate text-foreground">{{ $channel['channel'] }}</span>
