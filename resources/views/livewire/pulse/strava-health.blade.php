<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    ...
    <x-pulse::scroll :expand="$expand" wire:poll.30s="">
        <div class="space-y-4">
            ...
            <div>
                <div class="text-label-micro text-text-3 mb-1">Last {{ $this->periodForHumans() }}</div>
            ...
            <div>
                <div class="text-label-micro text-text-3 mb-1">Shared API Budget (whole app)</div>
                <div class="grid grid-cols-2 gap-2 text-center">
                    <div class="rounded-sm bg-muted p-1">
                        <div class="font-mono text-lg font-bold tabular-nums text-foreground">{{ $rateLimit['15min'] ?? '-' }}<span class="text-xs font-normal text-text-3">/200</span></div>
                        <div class="text-label-micro text-text-3">15 min left</div>
                    </div>
                    <div class="rounded-sm bg-muted p-1">
                        <div class="font-mono text-lg font-bold tabular-nums text-foreground">{{ $rateLimit['daily'] ?? '-' }}<span class="text-xs font-normal text-text-3">/2k</span></div>
                        <div class="text-label-micro text-text-3">daily left</div>
                    </div>
                </div>
            </div>
                        ...
                        @foreach ($perUser as $row)
                            <div @class([
                                ...
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    <span class="truncate text-foreground">{{ $row['user_name'] }}</span>
                                </div>
                                <div class="flex items-center gap-3 shrink-0 tabular-nums text-text-3">
                                    @if ($row['last_sync'])
