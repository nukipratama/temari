...
- `scripts/worktree remove` deletes the worktree and its `worktree-<name>` branch outright, so unpushed commits are discarded — push the branch before removing it. It never deletes a branch that `adopt` set up, because `create` did not make that branch.
