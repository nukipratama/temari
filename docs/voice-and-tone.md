...
temari (the character's name is lowercase, even at the start of a sentence — see [Register](#register) below) is a limbless, ball-bodied character with a face on its surface: brows, eyes, and a mouth that shift with mood. It moves by bouncing and rolling, never walking. The rendering lives in [TemariProto.tsx](../resources/js/components/temari/TemariProto.tsx), ported from the brand generator [build-mascot.mjs](../resources/brand/build-mascot.mjs), which draws all ten faces from one geometry so they share a skull.

Expression is 10 named faces — `resting`, `pleased`, `impressed`, `hyped`, `skeptical`, `unimpressed`, `challenging`, `concerned`, `disappointed`, `celebrating` — plus a **mood halo**: a closed ring around the body whose colour and weight carry the mood. The halo is never a fill, so it never reads as a progress meter. The 8 original pose names (`proud`, `pumped`, `excited`, `holding`, `reading`, `wobble`, `observational`, `glow`) remain valid and resolve to the face carrying the same read. Range matters here: temari keeps score, so `unimpressed`, `skeptical` and `challenging` are as much part of the character as `celebrating` is, and praise reads as earned precisely because the flat faces exist.

Thread coverage is the visual language of progress. The Plan tab's season summary is the one place this appears, tracking `PlannedSession.phase`: `base` (one sparse band) → `build` (widening bands) → `peak` (fully wound, most saturated) → `taper` (peak's full pattern retained, plus a rested shine — taper is progress kept, never undone). Self-scaled deload weeks pause accretion rather than reset it. Everywhere else the mascot has no coverage at all, so it never becomes phase-aware app-wide (`SeasonCoverage` in TemariProto.tsx).

Equipped accessories render onto the ball rather than a limbed body — same 6 slots and unlock catalog as before, only the rendering changed: `headband`, `shirt` and `shorts` are flat bands clipped to the body circle so they pick up the ball's curve and can never escape the silhouette, `shoes` sit under the body, `medal` hangs on a lace over it, and `aura` is a dashed ring outside the halo. Colour carries rarity; a small detail carries the item's theme.
