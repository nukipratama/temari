...
[RunCardFactory](../../app/Services/Run/Story/RunCardFactory.php) (`build(Activity, ActivityDetail): RunCard`) is the entry point, but it only orchestrates and persists: it resolves the PR flag (sticky, and never claimed while an earlier run is still awaiting hydration), delegates the **badges** (weather, distance bracket, splits, streak) and the **rarity** score, names a **special move**, writes the row and queues the reveal. It is invoked from the ingest pipeline ([ActivityPipeline](../../app/Services/Run/Ingest/ActivityPipeline.php)). A card's `pr_set` means *this run set a record on the day it was run, given every earlier run*. A backfill hydrates newest-first, so that can't be judged at ingest: once a run lands behind later ones with nothing earlier left to hydrate, [RecomputeCardClaimsAction](../../app/Actions/Run/Story/RecomputeCardClaimsAction.php) replays the athlete's history in date order and rebuilds every card whose flag was wrong, along with the post-run mood that reads it. `run:recompute-card-claims` ([RecomputeCardClaimsCommand](../../app/Console/Commands/Run/RecomputeCardClaimsCommand.php), `--dry-run` available) is the owner-run correction for cards minted before this existed.
