...
`force: true` bypasses recency and the master-switch opt-in, because the user explicitly asked for that send. It **cannot** bypass a channel mute: a mute is a routing decision ("never deliver here"), not a per-message one. This is the one gate force does not override, and it is pinned by a test.
