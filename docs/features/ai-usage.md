...
- A **budget gauge** ([BudgetGauge](../../resources/js/components/aiusage/BudgetGauge.tsx)) comparing the window's spend against the per-athlete ceiling and, in its own tile, against the enforced app-wide total, plus — once the ceiling has tripped today — the trip time and how many blocks were served rule-based as a result, recorded by [CostCeilingLedger](../../app/Services/AI/CostCeilingLedger.php).
