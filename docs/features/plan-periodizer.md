...
**Frontend**: [SeasonPhaseBar](../../resources/js/components/plan/SeasonPhaseBar.tsx) groups consecutive same-phase weeks into one bar segment, sized by that phase's share of season volume — a race-oriented season naturally yields 4 segments (Base/Build/Peak/Taper), a self-scaled one a repeating Build/Deload cycle (see `App\Enums\PlanPhase`'s own docblock on which phases exist in which mode), rather than forcing every season into a fixed 4-column shape. [SeasonWeekTimeline](../../resources/js/components/plan/SeasonWeekTimeline.tsx) renders a planned-vs-actual bar per week across the whole arc, collapsing weeks outside a small window around the current week behind a toggle (reusing the `Collapsible` primitive `S4` ported). Phase fill colors are a new `PHASE_COLORS` export in [chartTokens.ts](../../resources/js/lib/chartTokens.ts), validated distinct via the dataviz skill's palette checker and deliberately drawn away from `PALETTE.overloaded`/`gassed`/`chill`, which are already committed to per-run Mood colors.
