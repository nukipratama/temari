...
- **Notifications** — one section holding two groups, because the user's model is one topic with two questions rather than three unrelated ones:
    - _What gets sent_ — one channel-neutral master switch, **Keep me posted** (`notifications_enabled`), covering the post-run story, both recaps, the morning briefing push, the race-day-tomorrow reminder and the streak nudge, and gating Telegram and phone push alike. Full behaviour in [[telegram-notifications]].
