...
The headline says which reading carried the call, from the payload's `verdict_metric`, `pace_relation` and `hr_relation` ([improvingHeadline](resources/js/lib/verdict.ts#L191)). An `improving` window whose every pair was decided on efficiency reads "faster at the same heart rate" when the mean pace gain clears 2% and the mean HR moved no more than 2 bpm either way (`hr_relation` is `same`), or "same pace, lower heart rate" inside the pace band. A faster window whose HR moved further, or whose pace slipped by more than 2%, gets the neutral "running better" wording. A pace-decided window says "faster". A window that mixed efficiency and pace pairs gets neutral wording, so no line claims heart rate for a pair decided on pace. A `plateaued` window where most flat pairs got faster at a higher HR reads "faster, but it cost more heart rate." ([flatPairsCostHeartRate](resources/js/lib/verdict.ts#L92)). No efficiency number or percentage is rendered anywhere; the supporting line states the mean pace and HR deltas as raw facts.
...
[TodaySession](resources/js/components/home/TodaySession.tsx) is the one forward-looking block on an otherwise backward-looking page: Temari as a watermark beside the "Today" eyebrow, posed to the daily vibe (`briefing.mood`) and thinking while today's read is written; what today asks for at full width, then the voice describing it, on a `today-accent` edged card (`PS3` moved it off the `sky` panel it used to sit on, see [[design-tokens]]).
