...
- **`strava:remove-athlete {user}`** ([RemoveAthleteCommand](../../app/Console/Commands/Strava/RemoveAthleteCommand.php)) releases one athlete and revokes the local connection while keeping their account and runs — a slot freed without deleting anyone. It refuses the demo account, and revokes locally even when Strava declines, since the operator's intent was for that athlete to stop syncing either way.
