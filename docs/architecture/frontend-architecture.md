...
- **Mounts under a shared [ErrorBoundary](resources/js/app.tsx#L77)** and installs the global client-error reporter ([`installGlobalErrorReporting`](resources/js/app.tsx#L18)) that POSTs to the CSRF-exempt `/client-errors` sink. Inertia's own progress bar is **off** ([`progress: false`](resources/js/app.tsx#L85)); the app draws its own [`RouteProgressBar`](resources/js/layouts/AppShell.tsx#L35) inside the shell instead, so it can carry app tokens ([[design-tokens]]) and sit under the top bar.
