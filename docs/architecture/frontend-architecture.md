...
Two routes carry the `inertia-etag` alias ([bootstrap/app.php](bootstrap/app.php), [routes/web.php](routes/web.php)): `/history` and `/activities/{activity}`. [`SetInertiaEtag`](app/Http/Middleware/SetInertiaEtag.php) tags their full Inertia page object with `ETag` + `Cache-Control: private, no-cache`, so a revisit revalidates and gets an empty `304` when nothing moved. It is applied **per-route, never globally** — it saves wire bytes, not server work (the controller has already run and Inertia has already serialized the page by the time the middleware sees the response), so it only pays off where the same URL is genuinely re-fetched: filter, tab and view toggling on `/history` (list and calendar both), and deep links back into a past run from a Telegram notification or a share URL. A run detail carries `stream_summary` + `summary_polyline` + the card, and is the larger of the two now that `/history` defers its list (see below).
...
**`/history` now tags a shell, not a page.** Its run list, notes, moods, weekly snapshots (and on the calendar branch its cells and monthly recap) moved behind `Inertia::defer()`, so they are not in the tagged response at all — the tagged body fell from ~10.2 KB to ~1.3 KB, and the ~9.4 KB that used to ride inside it now arrives on a partial request, which is `no-store` by the rule below. A revisit that used to revalidate to an empty `304` therefore still gets that `304` for the shell but re-fetches the deferred group every time. The guarantee narrowed rather than broke — nothing stale is served, because the volatile data is no longer in the cached response — but the wire-bytes saving this alias was added for is now much smaller on `/history` than on `/activities/{activity}`, which still ships its `stream_summary` + `summary_polyline` + card eagerly.
