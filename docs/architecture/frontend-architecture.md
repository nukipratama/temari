...
- **Mounts under a shared [ErrorBoundary](resources/js/app.tsx#L27)** and installs the global client-error reporter ([app.tsx:9](resources/js/app.tsx#L9)) that POSTs to the CSRF-exempt `/client-errors` sink. The Inertia progress bar uses the leaf green ([app.tsx:32](resources/js/app.tsx#L32)); tokens live in [[design-tokens]].
...
With React pinned, the entry's static closure is the Rolldown runtime + `app` + `react-vendor` only — **371.5KB raw / 118.7KB gzipped, down from 693.8 / 222.0**. `charts` and `maps` are additionally behind `lazy()` boundaries ([FitnessTrend.tsx:20](resources/js/components/trends/panels/FitnessTrend.tsx#L20), [Show.tsx:54](resources/js/pages/Runs/Show.tsx#L54)), so neither sits in *any* route's static closure. The four bottom-nav chunks are warmed after idle ([app.tsx:44](resources/js/app.tsx#L44)).
