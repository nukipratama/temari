...
- **Two grounds, Tailwind v4, dark by default.** The ground is a `data-theme` attribute on `<html>` — never a `.dark` class — resolved before first paint by a blocking inline script in [app.blade.php](resources/views/app.blade.php#L29) that reads a `temari-theme` localStorage key, so there is no flash and guests on Login get it too. `system` tracks `prefers-color-scheme` live via [`useSystemTheme`](resources/js/hooks/useSystemTheme.ts#L13). A ground-reactive semantic layer sits above the fixed named palette; both are in `resources/css/app.css`'s `@theme` block, see [[design-tokens]].
