...
With React pinned, the entry's static closure is the Rolldown runtime + `app` + `react-vendor` only — **371.5KB raw / 118.7KB gzipped, down from 693.8 / 222.0**. `charts` and `maps` are additionally behind `lazy()` boundaries ([FitnessPanel.tsx:24](resources/js/components/trends/panels/FitnessPanel.tsx#L24), [Show.tsx:54](resources/js/pages/Runs/Show.tsx#L54)), so neither sits in *any* route's static closure. The four bottom-nav chunks are warmed after idle ([app.tsx:44](resources/js/app.tsx#L44)).
