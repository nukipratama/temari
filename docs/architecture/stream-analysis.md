...
- **Personal records** — distance PRs slide their window over `per_km` on `elapsed_sec`, then the `partial_split` leftover, so a 42.6 km run can still reach the 42 195 m marathon target ([splitRows](app/Services/Run/Metrics/PersonalRecords.php#L134)). The `/records` scoreboard's pace strip reads the same two keys ([featuredExtras](app/Services/Run/PrScoreboardBuilder.php#L48)) — which is why [RekorController](app/Http/Controllers/RekorController.php#L33) has to name `stream_summary` in its eager-load column list. See [[records]].
