...
It's written during ingest by [ActivityPipeline::lookupWeather](app/Services/Run/Ingest/ActivityPipeline.php#L480): it uses the detail's stored `start_lat` / `start_lng` and `start_date_local`, then calls the client. Consistent with the pipeline's best-effort contract ([class header comment at ActivityPipeline.php:44](app/Services/Run/Ingest/ActivityPipeline.php#L44)), a missing coord/time short-circuits and a thrown lookup is caught and logged so weather can never strand an activity as an un-ingestable stub ([catch at ActivityPipeline.php:493](app/Services/Run/Ingest/ActivityPipeline.php#L493)). The same start point feeds [[geo-reverse-geocoding]], which populates `location_name`.
