...
- **[[dashboard]]** — the last-run weather chip on [LastLariCard](resources/js/components/dashboard/LastLariCard.tsx#L24), via the `formatWeather` helper. [TodayHeroBanner](resources/js/components/dashboard/TodayHeroBanner.tsx#L89) uses the same helper but is no longer mounted on any route (see [[dashboard]]).
