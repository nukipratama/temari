...
It's written during ingest by [ActivityPipeline::lookupWeather](app/Services/Run/Ingest/ActivityPipeline.php#L492): it uses the detail's stored `start_lat` / `start_lng` and `start_date_local`, then calls the client. Consistent with the pipeline's best-effort contract ([class header comment at ActivityPipeline.php:38](app/Services/Run/Ingest/ActivityPipeline.php#L38)), a missing coord/time short-circuits and a thrown lookup is caught and logged so weather can never strand an activity as an un-ingestable stub ([catch at ActivityPipeline.php:457](app/Services/Run/Ingest/ActivityPipeline.php#L457)). The same start point feeds [[geo-reverse-geocoding]], which populates `location_name`.
