...
- **[[dashboard]]** — the last-run weather chip on [LastLariCard](resources/js/components/dashboard/LastLariCard.tsx#L24), via the `formatWeather` helper. It is now the only consumer: the hero banner that also showed a weather chip was deleted with the old dashboard (see [[dashboard]]).
