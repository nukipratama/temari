...
- **[[run-detail]]** — `MapWeatherPanel` on [Runs/Show.tsx](resources/js/pages/Runs/Show.tsx#L245) renders the temp / humidity / wind / location block beside the route map. (A richer standalone [WeatherHero](resources/js/components/aktivitas/WeatherHero.tsx) card also exists, keyed off the same seven fields.)
