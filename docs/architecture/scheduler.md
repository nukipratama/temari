...
|---|---|---|---|---|---|
| `notifications:recover-deliveries` | every 5 minutes | 10 | yes | one indexed scan of stale pending claims; versioned updates fence concurrent workers, and the command makes no provider calls | not measured — added with fenced notification recovery |
