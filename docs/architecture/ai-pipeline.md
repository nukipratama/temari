...
**Queue.** AI jobs run on their own queue (`ai.queue`, default `ai`) under the `supervisor-ai` Horizon supervisor ([config/horizon.php](config/horizon.php)), which allows a 300 s timeout against the 60 s the rest of the queue lives by — a tool-calling run takes several Azure round trips. Production splits the container's 4 workers 2/2, so a narration backlog can never occupy every worker and stall Strava ingest behind it.
