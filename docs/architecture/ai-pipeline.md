...
- **Idempotency at upsert** — a row is created `Queued` (or `Pending` when dispatch is off) only `wasRecentlyCreated`; an existing row is re-dispatched only when `claimForDispatch()` wins it — one conditional `UPDATE ... WHERE status IN ('pending','failed')` that asks and takes in the same statement, so two dispatchers racing the same row cannot both enqueue (see [[dispatch-claims-the-row]]). A `Done` or `Queued` row is left alone, so a same-day re-run of the daily briefing dispatches only the still-missing types.
