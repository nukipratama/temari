...
description: How AI copy flows from a narrator through a queued job into an Analysis row, with cadence, chaining, idempotency, cost ceilings, bounded self-heal, manual retry, and dead-lettering.
...
- **[AnalyzeGroupJob](app/Jobs/AI/AnalyzeGroupJob.php)** carries `(subjectId, discriminator)` and narrates several rows of one subject together. [AnalyzeActivityJob](app/Jobs/AI/AnalyzeActivityJob.php) is the only group: it persists run-insight claims as soon as that narrator returns, then generates post-run speech when the shared job deadline has room. If speech cannot fit its worst-case call, it remains Pending for self-heal while insight stays Done. Which types belong to a group is the single source of truth `AnalysisType::groupJobClass()`; `AnalyzeGroupJob::groupedTypes()` derives from it. Grouping matters for cost: the speech reuses the already-Done insight row verbatim, so a speech-only re-run never re-bills the insight (`AnalyzeActivityJob::resolveInsight()`).
...
- **Atomic attempts** — each real run increments `attempts` in the database instead of writing a value calculated from a stale model, so concurrent claims cannot lose an increment.
...
- **Daily cost ceiling, per athlete** — `autoDispatchEnabled()` splits into `dispatchAllowedIgnoringBudget()` and `dailyCostCeilingExceeded()`. `azure_openai.daily_cost_ceiling_per_user` defaults to **$1.00/athlete/day** (an operator can set a today-only override per athlete); the athlete is resolved from the row's subject via [AnalysisSubjectMap::ownerId()](app/Services/AI/AnalysisSubjectMap.php) at both dispatch sites. Once *that athlete's* spend today exceeds it, auto-dispatch is skipped until midnight resets the daily cost, and — because the budget is the *only* stop that resolves on a clock rather than on a fix — a `Pending` row is filled from the rule-based filler and marked `Done` instead of resting `Pending` (a `Failed` row is excluded, so a real fault keeps its dead-letter visibility) (`AnalysisService::costCeilingDegraded()` / `degradeToRuleBased()`, and the same branch in `haltForPausedGeneration()` for an already-queued job). Every other stop still pauses to `Pending`; see [[cost-ceiling-degrades-to-rule-based]]. The trip time and the resulting fill count are recorded per day by [CostCeilingLedger](app/Services/AI/CostCeilingLedger.php) and shown on `/devtools/narration`. A null ceiling never gates. The `analytics` aggregate behind it is memoized per scope *and per athlete*: `AnalysisService` is a `scoped` binding, so one HTTP request or one queue job reads a given athlete's day spend once however many rows it fans out over (an ingest asked ~6 times before); a scope touching several athletes reads one aggregate each. Only the cost read is memoized — the kill switch and the config breaker stay live, so a breaker reset still resumes generation mid-scope. Nothing bills inside that scope: dispatch only enqueues, and the gate that actually guards spend is `haltForPausedGeneration()` in the analyze job, which runs in a separate process with its own fresh read. Manual triggers stay refused past the athlete's own ceiling (`generationPaused($userId)` still counts it), so the degrade is not a bypass. Above the per-athlete slice sits the app-wide `azure_openai.daily_cost_ceiling_total` (default **$5.00/day**), which `dailyCostCeilingExceeded()` checks first: past it every athlete degrades the same way, callers with no athlete in hand (the /pulse status line, `ai:self-heal`) are paused too, and one maintainer alert fires per hour; see [[app-wide-ceiling-above-the-per-athlete-one]]. A replay (`AnalysisOrigin::Replay`) is measured against its own `azure_openai.replay_daily_cap` and the total, never the athlete's slice.
- **Per-block agent budget** — the ceiling above is read once, *before* the job is queued, so it cannot stop a tool-calling narration mid-run. `ai.agent.max_steps` / `ai.agent.max_tokens` in [config/ai.php](config/ai.php) bound one block instead; see [[narration-agents-on-openai-php]]. `max_steps` is only the **default**: it is sized for the widest toolbox (RunInsight's ten tools), and every extra turn re-bills the whole prompt prefix including the persona, so a narrator holding one or two tools tightens it through `ChatCallOptions::$maxSteps` ([ChatCallOptions](app/Services/AI/ChatCallOptions.php) → [AgentBudget::fromConfig](app/Services/AI/Agent/AgentBudget.php)). The floor for an override is `2 * (tools + 1)` — one pass is at most `tools + 1` turns since every tool is argument-free and worth calling once, and the content-filter retry replays that pass on the same budget, so anything tighter would have the retry answer with no readings at all.
- **Wall-clock deadline** — steps and tokens bound what a run *costs*, not how long it *holds a worker*: `max_steps` alone allows `max_steps * azure_openai.timeout` (10 x 90 s = 900 s) against a 360 s worker timeout, and a worker kill lands mid-request, with the step billed and the block empty. `ai.agent.deadline_seconds` ([config/ai.php](config/ai.php), 240 s) is checked before every Azure request ([AgentLoop](app/Services/AI/Agent/AgentLoop.php), [AgentBudget](app/Services/AI/Agent/AgentBudget.php)). Activity groups share one deadline from job start with a 10 s safety margin; each narrator receives only the time left, and speech is skipped unless its full configured deadline plus one request timeout fits. Insight is committed before speech starts, so a timeout or skip cannot erase it. A single narrator can overshoot its deadline by the request already in flight, so `deadline_seconds + azure_openai.timeout + safety margin` must stay under the supervisor timeout; [AgentDeadlineFitsWorkerTest](tests/Unit/Architecture/AgentDeadlineFitsWorkerTest.php) checks the single-call and multi-call bounds, and that `queue.connections.redis.retry_after` stays above every worker timeout. The per-request timeout is *not* narrowed to the remaining budget: a cut-off request surfaces as a `TransporterException`, which the taxonomy reads as transient and retries — the opposite of giving up.
...
**Queue.** Every analyze job declares `$queue = 'ai'` on [AnalyzeBaseJob](app/Jobs/AI/AnalyzeBaseJob.php) — not configurable, since there is one right answer. The `supervisor-ai` Horizon supervisor ([config/horizon.php](config/horizon.php)) serves it at a 360 s timeout against the 60 s the rest of the queue lives by, because a tool-calling run takes several Azure round trips. That timeout is the outer safety margin behind the run's own wall-clock deadline (above), not the real bound. Production splits the container's 4 workers 2/2, so a narration backlog can never occupy every worker and stall Strava ingest behind it.
...
A `Failed` block gets a bounded automatic retry from `ai:self-heal` (above) and then dead-letters; after that only a person re-arms it. The bound, not a no-retry rule, is what keeps LLM cost predictable. See [[bounded-self-heal-and-dead-letter]] and [[per-block-manual-retry]].
...
1. **Every type is narrated.** The run-insight blocks were the last types filled inline from arithmetic; they go through the model now, so there is no longer a no-queue, no-token path in `dispatchRow()`. **A real account's block is templated only past a cost ceiling.** When generation is paused (AI off / Azure unset / config breaker tripped) a single-row LLM block stays honestly `Pending` (an existing Done keeps its real prose) and `ai:self-heal` fills it once generation resumes. Past the per-athlete or app-wide cost ceiling a `Pending` block is instead filled rule-based and marked `Done` (see the ceiling bullet above). A block that reaches the LLM and keeps failing is bounded by `Analysis::MAX_SELF_HEAL_ATTEMPTS`, then **dead-lettered** — surfaced per-user on `/devtools/narration` ([NarrationOverviewController](app/Http/Controllers/NarrationOverviewController.php)) with a manual "Try again" that re-arms the budget.
2. **Demo seed** — the demo seeder backfills every Analysis row through [RuleBasedNarrationFiller](app/Services/AI/RuleBased/RuleBasedNarrationFiller.php) under `AnalysisService::withoutDispatching()`, so seeding spends no tokens (on a real account the filler runs only past a cost ceiling). The "Reread" button stays live for the demo, but its trigger is filled rule-based instead of dispatching, so it spends no tokens either — see [[demo-triggers-served-rule-based]].

## See also

- [[llm-triggers]] — the other half of this note: the full inventory of what can call a model, who *starts* a dispatch, and everything that stops one.
