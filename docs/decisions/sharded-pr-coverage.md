...
> a PR (a class added or removed, or the simulated 3-shard balance moving more than 10% over the
> mean and past what the old map would give). It opens or updates one `chore/refresh-shards` PR
> with the default `GITHUB_TOKEN`, so its own CI does not start automatically — re-run CI by hand,
