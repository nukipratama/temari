...
**Temari is a training partner who keeps score.** Still warm, now competitive about the runner's own numbers, and willing to name it when they are coasting. The full register lives in the `SYSTEM_PROMPT` at [`SYSTEM_PROMPT`](../../app/Services/AI/TemariPersona.php#L29); it is the single source of truth and this note does not restate it.

- **Three new prompt sections carry the shift.** `# Keeping score` ([the "Keeping score" section](../../app/Services/AI/TemariPersona.php#L57)) enumerates the legal scoreboard axes and requires naming the number and the direction, including when the direction is bad. `# Calling a coast` ([the "Calling a coast" section](../../app/Services/AI/TemariPersona.php#L80)) licenses the honest read once, plainly, and pairs it with a hard exclusion list. `# Praise is earned, never issued` ([the "Praise is earned" section](../../app/Services/AI/TemariPersona.php#L105)) reframes encouragement as a scarce currency and states that a block with nothing to praise is already finished.
...
- **Emoji tighten to one per output, default zero, PRs and first-evers only** ([the emoji policy](../../app/Services/AI/TemariPersona.php#L217)). The product already carries celebration visually through the mascot's expression states, rarity cards, and auras, so a text emoji says a second time what the screen already said.
...
- **Deterministic fallback copy counts as persona surface.** The four hardcoded ceiling-violation strings at [`clampedVoice()`](../../app/Services/AI/Narrators/BriefingMascotVoiceNarrator.php#L367) ship to users whenever the model exceeds its readiness ceiling, so they were rewritten alongside the prompts rather than left as a visible register fork.
