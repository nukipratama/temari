...
- **One pool, two ceilings.** [`guardRateLimit()`](app/Services/Strava/StravaClient.php#L369) still hits the same globally-keyed buckets, but a `Background` read is refused once a bucket reaches [`backgroundCeilings()`](app/Services/Strava/StravaClient.php#L406) — at the time of this decision `100 - LIVE_RESERVE_PERCENT` of the max, i.e. 150 / 15 min and 1,500 / day. `Live` may spend the whole pool.
