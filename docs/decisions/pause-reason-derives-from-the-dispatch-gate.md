...
**1. One ordered list, two callers.** `blockingReason()` ([`blockingReason()`](app/Services/AI/AnalysisService.php#L1008)) returns the first condition stopping a dispatch, or `null`. `pauseReason()` ([`pauseReason()`](app/Services/AI/AnalysisService.php#L984)) *is* that list; `autoDispatchEnabled()` ([`autoDispatchEnabled()`](app/Services/AI/AnalysisService.php#L989)) and `dispatchAllowedIgnoringBudget()` ([`dispatchAllowedIgnoringBudget()`](app/Services/AI/AnalysisService.php#L994)) ask whether it returned `null`. Adding a stop condition changes one list and both answers follow, so the reported reason cannot drift from the decision again.
...
- **The budget is optional.** `costCeilingDegraded()` ([`costCeilingDegraded()`](app/Services/AI/AnalysisService.php#L1044)) must know whether the ceiling is the *only* stop, per [[cost-ceiling-degrades-to-rule-based]], so it asks the list to leave the budget out. `withBudget: false`.
