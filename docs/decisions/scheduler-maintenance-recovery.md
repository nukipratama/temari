...
- `streak:settle` stores `users.streak_settled_through` plus a dirty-from marker lowered by writes to already-settled weekly snapshots. A null cursor or actionable dirty marker causes a chronological rebuild of available weekly history, replacing derived rest-token outcomes atomically for that rebuild; later recovery is bounded and self-dispatches until the cursor reaches the latest closed week. Weekly recap creation and filling remain gated while any athlete is behind.
