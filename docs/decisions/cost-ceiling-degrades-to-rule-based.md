...
Meanwhile the filler that already exists for the demo seed, [RuleBasedNarrationFiller](app/Services/AI/RuleBased/RuleBasedNarrationFiller.php), is an **exhaustive `match` over every `AnalysisType`** ([RuleBasedNarrationFiller.php:39](app/Services/AI/RuleBased/RuleBasedNarrationFiller.php#L39)) and is data-driven where the subject's real numbers are available, so there is no coverage gap and no "generic template" tell.
