...
Meanwhile the filler that already exists for the demo seed, [RuleBasedNarrationFiller](app/Services/AI/RuleBased/RuleBasedNarrationFiller.php), is an **exhaustive `match` over every `AnalysisType`** ([`fillFor()`](app/Services/AI/RuleBased/RuleBasedNarrationFiller.php#L59)) and is data-driven where the subject's real numbers are available, so there is no coverage gap and no "generic template" tell.

## Decision

**1. The cost ceiling degrades.** Past the ceiling, a `Pending` block is filled from the rule-based filler and marked `Done` instead of resting `Pending` — at dispatch time in `AnalysisService::degradeToRuleBased()` (both the row and the group path), and again in `AnalyzeBaseJob::haltForPausedGeneration()` for a job that was already queued when the ceiling tripped ([`haltForPausedGeneration()`](app/Jobs/AI/AnalyzeBaseJob.php#L203)). Fills run under `withoutDispatching()`, so no job is queued, no cooldown starts, and no notification claims a narration that was never written.
