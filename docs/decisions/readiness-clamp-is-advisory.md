...
- **Scoring.** A nullable `rest_clamped_at` on `planned_sessions`, written wherever the ceiling is already computed — the ingest listener and the 00:01 briefing, never on a render — letting the scorer resolve a clamped-to-Rest day to `Skip`: uncredited, but never penalising adherence. Only the full-Rest clamp needs this. `Long -> Easy` is a fixed `SegmentGenerator::MEDIUM_FRACTION_OF_LONG` of the long run, clear of `SessionMatcher::PARTIAL_FRACTION`, so complying already credits; Tempo/Interval downgrades keep the same core km.
