...
- **It runs the kickoffs' own creation code**, under
  ...
  suppression short-circuits [`blockingReason`](../../app/Services/AI/AnalysisService.php#L1046),
  which reduces `dispatchRow` to its `firstOrCreate` in [`upsertRow`](../../app/Services/AI/AnalysisService.php#L688): a missing row is created `Pending`, an
