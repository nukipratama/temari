...
**2. Both ceilings are one code path.** [`dailyCostCeilingExceeded()`](app/Services/AI/AnalysisService.php#L1123)
asks the total first and the athlete's own slice second, both through the same
[`ceilingExceeded()`](app/Services/AI/AnalysisService.php#L1185) helper. Sharing the path is the point: what "past the ceiling" *does* can never diverge between the two. So
...
([`shouldServeRuleBased()`](app/Services/AI/AnalysisService.php#L944)) so it is
