...
Persist every accepted Telegram `update_id` in `telegram_update_receipts` before dispatching its job. The primary key makes the insert the atomic dedupe claim. The webhook acknowledges duplicates without dispatch; `telegram:listen` advances its local offset but skips already-received updates. Receipts older than seven days are pruned daily.

Keep link tokens stateless and signed, but persist a unique SHA-256 token hash in `telegram_link_token_uses` when it is consumed. The claim and connection update run in one database transaction, so only one concurrent start can win and a failed link rolls back its claim. The winning transaction dispatches a separate retryable welcome job after commit, so a Telegram send retry never repeats the link. Expired token-use claims are pruned daily.
