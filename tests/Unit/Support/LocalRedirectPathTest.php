...
it('rejects anything that is not a bare relative path', function (?string $path): void {
    ...
    'null' => null,
    'absolute http' => 'http://evil.test/activities/5',
    'absolute https' => 'https://evil.test/activities/5',
    'protocol relative' => '//evil.test/activities/5',
    'no leading slash' => 'activities/5',
...
it('drops an intended URL pointing at another host', function (): void {
    expect(LocalRedirectPath::fromIntended('https://evil.test/activities/5'))->toBeNull();
});
