...
class AnalysisService
{
    ...
    public function markDone(
        ...
        // row, a paused-generation degrade) is never regenerated by the
        // replay. $startedEarly (the state generation began under, passed by
        // the caller) is ORed with a fresh check so a row that straddles the
        // drain finishing mid-generation is still caught.
        $early = $servedBy === ServedBy::Llm && ($startedEarly || $this->isEarlyPassRow($row));
