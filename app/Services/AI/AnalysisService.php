...
class AnalysisService
{
    ...
    public function markDone(
        ...
        // Only an LLM serve can be "early": a rule-based fill (an old backfill
        // row, a paused-generation degrade) is never regenerated by the replay
        // below, so it must never claim the marker.
        $early = $servedBy === ServedBy::Llm && $this->isEarlyPassRow($row);
        ...
        $row->update([
            ...
            // Stamped while this row's history is still hydrating (a fresh
            // connect's early pass); cleared by an ordinary re-narration once
            // that history has landed. See SettleEarlyNarrationAction.
            'narrated_early_at' => $early ? Carbon::now() : null,
        ]);

        if ($early) {
            $ownerId = AnalysisSubjectMap::ownerId($row->subject_type, $row->subject_id);
            if ($ownerId !== null) {
                User::query()->whereKey($ownerId)->whereNull('history_replay_due_at')->update([
        ...
        // can't run before the row it reads is committed. Also suppressed while
        // $early: every channel's delivery claim is keyed on this row's id for
        // good, so notifying now would permanently spend it on a run that
        // cannot yet know whether it set a PR — the replay's regeneration is
        // the row's one real send.
        if (! $this->dispatchSuppressed
            && ! $early
     ...
     */
    private function isEarlyPassRow(Analysis $row): bool
    {
        return match ($row->analysis_type) {
            AnalysisType::PostRunSpeech, AnalysisType::RunInsight => $this->earlyPassForActivity($row->subject_id),
    ...
    private function earlyPassForActivity(int $activityId): bool
    {
    ...
    private function earlyPassForCard(int $cardId): bool
    {
